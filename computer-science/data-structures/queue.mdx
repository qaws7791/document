---
title: Queue
description: 큐는 시퀀스의 한쪽 끝에서 요소를 추가하고 다른 한쪽 끝에서 요소를 제거하는 자료형입니다.
---


## 큐(Queue)란

큐(Queue)는 **선입선출(First In First Out, FIFO)** 방식에 따라 요소를 추가하고 제거한다. 따라서 요소는 큐의 마지막으로만 추가되며, 큐의 첫 부분으로만 나갈 수 있다. 큐는 먼저 들어간 것이 먼저 나오므로 차례대로 처리하는 일상의 줄 서기와 비슷하다
